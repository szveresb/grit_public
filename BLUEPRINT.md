...
**Questionnaire Categories & Taxonomy filtering implemented.** Added schema migration, seeded default categories (`mood_state`, `wellbeing`, `relationships`, `caregiving`), mapped existing questionnaires, and updated TypeScript database interfaces. Added bilingual localization keys. Updated the card component to display category pill badges, and updated the filler component with mobile-friendly horizontal scrolling topic chips and deactivation fallbacks. Enabled category pills for public preview list items and details. Added category select options, pre-publish validation, cloning support, and a complete admin Category CRUD management overlay on the self-checks administration page.
...
**Survey grouping by category implemented in /surveys.** Grouped survey cards under localized category headings in both public and authenticated views when no specific category is selected, keeping groupings isolated within their respective sections (Self vs. Observer). Enabled fallback flat layout when category filter is active, filtered out empty categories, and handled uncategorized questionnaires under a fallback heading. Tested with TypeScript compiler verification.
...
**Questionnaire Answers Display in History fixed.** Modified `ScoreHistory.tsx` to remove the invalid nested relation `.order('questionnaire_questions(sort_order)' as never)` clause on the `questionnaire_answers` database query, which was causing 400 Bad Request errors. Replaced it with clean database retrieval and error checks while preserving the correct client-side sorting flow. Checked compile safety with 0 errors.
